...
def cargar_datos():
    with zipfile.ZipFile("parte_1.csv.zip") as z:
        nombre1 = [n for n in z.namelist() if n.endswith(".csv") and "__MACOSX" not in n][0]
        df1 = pd.read_csv(z.open(nombre1), parse_dates=["date"])

    with zipfile.ZipFile("parte_2.csv.zip") as z:
        nombre2 = [n for n in z.namelist() if n.endswith(".csv") and "__MACOSX" not in n][0]
        df2 = pd.read_csv(z.open(nombre2), parse_dates=["date"])
